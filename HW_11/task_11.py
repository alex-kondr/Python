...
class Vector:
    ...
    def __call__(self, value=1):
        self.coordinates.x *= value  # type: ignore
        self.coordinates.y *= value  # type: ignore
    ...
    def len(self):
        return (self.coordinates.x ** 2 + self.coordinates.y ** 2) ** 0.5  # type: ignore
...
class Iterable:
    def __init__(self, max_vectors, max_points):
        ...
        self.vectors = [
            Vector(Point(randrange(max_points), randrange(max_points))) for _ in range(max_vectors)
        ]
